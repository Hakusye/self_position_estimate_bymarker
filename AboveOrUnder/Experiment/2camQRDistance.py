...
if __name__ == "__main__":
    ...
    while True:
        ...
        frameL = cv2.resize(frameL, (int(frameL.shape[1]/2), int(frameL.shape[0]/2)))
        frameR = cv2.resize(frameR, (int(frameR.shape[1]/2), int(frameR.shape[0]/2)))
    
        cv2.imshow('frameL', frameL)
        cv2.imshow('frameR', frameR)

        if(Z != preZ):
            print(str(pixToum * Z / cmToum) + "cm")
        preZ = Z
